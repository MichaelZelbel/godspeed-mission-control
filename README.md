...
Start with Chapter 1 in the book, then install Hermes and Mission Control in Chapter 2. The installers fetch current public files. Chapter 2 includes a request to merge this edition's shared starter guidance into the installed mission control while preserving your own material. Copying starter-godspeed/ alone does not install the commands, permissions or skill discovery.
...
The matching starter instructions file useful updates during conversation and check relevant old facts while helping with a job. Clear notes do not wait for a second sorting request. Between-session checks still need a configured schedule. These instructions apply to the assistant using your mission control; Hermes is the book's example installation.

Sam's original interview files are not included or installed. Robin's fictional material is supplied only under `practice/maintenance-classes/`, outside the installed starter godspeed. Use a separate practice folder when trying it. Examples in the guides are never personal facts to import.
...
- practice/maintenance-classes/: fictional material for a separate practice godspeed.
...
- starter-godspeed/: shared instructions, format guides and starting folders.
