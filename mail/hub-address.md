# Give your hub its own email address

Optional. About ten minutes. Free for light use.

Your hub gets an address of its own, for example `sam-hub@agentmail.to`. You forward the
...
```
hub-mail connect agentmail
```

It asks for the address and the key. The key stays hidden while you type and is kept in your
hub's locked store, where every assistant's copy of `hub-mail` finds it.

## 4. Try it

Forward one ordinary email to the address, wait a minute, and ask your hub:
"Check the hub's own inbox and tell me what arrived."

## Stop it

```
hub-mail disconnect agentmail
```

The hub stops reading at once. The inbox and its mail stay at AgentMail, because services you
