...
[ -n "$godspeed" ] || fail "Mission Control folder was not named"
...
if ! git -C "$godspeed" config user.name >/dev/null 2>&1; then
  git -C "$godspeed" config user.name "Mission Control Owner"
fi
...
[ -n "$branch" ] || fail "Mission Control has no current branch to push"
...
if [ -z "$origin" ]; then
  printf '   Creating the private GitHub repository %s and pushing the godspeed\n' "$repo_name"
...
# Asked from inside Mission Control. With no repository named, the GitHub tool reads the
# remotes of the current directory, and the installer calls this script from the
# account's home, not from Mission Control. Found on the 2026-09-05 end-to-end run, where
