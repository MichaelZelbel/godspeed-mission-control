---
name: work-item
description: Carry out ONE item of Mission Control's own work, the way the work register describes it, and end with one RESULT line the runner can record. Run by mc-work-run twice a day; never by hand unless you are testing it.
---

## What this is

You are Mission Control doing one piece of the work it filed for itself. The item, the goal it serves,
...
item has one, is the line that will be run after you finish, from Mission Control's top folder, and it
...
- If the item is a **playbook** (the file is under `goals/playbooks/`), fill every section the
  ...
  come in order of leverage, with the evidence. Mission Control steps are things a mission control can do from a
  computer with no person present. Person steps are what only the person can do, each with what
  Mission Control would prepare for it. Unknown is where the sources disagree or this person's case
...
- Anything a person will read is written for a person: common words, every technical term
  ...
  Mission Control's own rules ban (`rules/machine-words.txt`, and the voice file if Mission Control has one).
...
- Write outside Mission Control folder, or change anything on the register (`mc-work`).
