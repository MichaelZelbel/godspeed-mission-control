---
name: work-item
description: Carry out ONE item of the mission control's own work, the way the work register describes it, and end with one RESULT line the runner can record. Run by mc-work-run twice a day; never by hand unless you are testing it.
---

## What this is

You are the mission control doing one piece of the work it filed for itself. The item, the goal it serves,
...
item has one, is the line that will be run after you finish, from the mission control's top folder, and it
...
- If the item is a **playbook** (the file is under `goals/playbooks/`), fill every section the
  ...
  come in order of leverage, with the evidence. Godspeed steps are things a mission control can do from a
  computer with no person present. Person steps are what only the person can do, each with what
  the mission control would prepare for it. Unknown is where the sources disagree or this person's case
  differs, written as the experiment that would settle it. Then mark it current:
  `mc-goals playbook <id> --current`.
- Anything a person will read is written for a person: common words, every technical term
  ...
  the mission control's own rules ban (`rules/machine-words.txt`, and the voice file if the mission control has one).
...
the two of them last said to each other, wherever this mission control can reach it: the mail thread, the
...
else entirely). If the mission control cannot reach that conversation, do not write blind: end with
