# work - what your mission control is doing, from planned to verified

**This room starts empty, and an empty one costs you nothing.** It fills the first time your mission control
...
OWNER: mission control            mission control, person, or company:<slug>
...
- **`attempt --ok` makes it attempted, never verified.** `verify` runs the CHECK and accepts only
  ...
  the mission control owns, the word "done" is not evidence and is refused in those words.
...
- **Time passes even when nothing runs.** `mc-work tick` releases dead leases, names what is due
...
mission control cannot do it at all from this machine). A mission control that says "I cannot read that page from here"
is worth more than one that quietly does something adjacent and reports success.

## The commands

```
mc-work file --what "..." --done-when "..." [--check "..."] [--goal G] [--key K]
mc-work take <id> --runner <name> [--approved-by "<your words>"]
mc-work attempt <id> --runner <name> --ok --result "..." | --failed "why"
mc-work verify <id> [--evidence "what was observed, where"]
mc-work block <id> --needs ... | unblock <id> --why ... | cancel <id> --why ...
