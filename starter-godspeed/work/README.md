...
OWNER: godspeed            godspeed, person, or company:<slug>
...
- **`attempt --ok` makes it attempted, never verified.** `verify` runs the CHECK and accepts only
  ...
  Mission Control owns, the word "done" is not evidence and is refused in those words.
...
godspeed cannot do it at all from this machine). A mission control that says "I cannot read that page from here"
