...
your hub what something is actually for.
...
no notifications. If the only thing your hub reads at 6am is what has a date on it, then what has
a date on it is your whole life as far as your hub is concerned.
...
`SERVES` links a card to the one above it. That link is what lets your hub answer "why am I doing
...
## A change keeps its reason
    ...
    hub-goals change <id> --set "DEADLINE=2027-12-31" --why "I moved the checkpoint"
...
`hub-goals check` refuses a diagnosis that claims a constraint without saying what would disprove
it, because that is not a diagnosis, it is a preference with a heading on it.

## The commands

```
hub-goals file --kind outcome|strategy|project|commitment --title "..." --source "..."
hub-goals adopt|pause|retire|achieve <id> --why "..."
hub-goals change <id> --set "FIELD=value" --why "..."
hub-goals progress <id> --evidence "..."       what exists now that did not before
hub-goals question <id> --text "..."           one in seven days, never two at once
hub-goals answer <id> --text "<your words>"
hub-goals diagnose <id>                        what is limiting it, on evidence
hub-goals attention                            who gets attention today, and why
hub-goals list | show <id> | tree | check
```

Chapter 6 of the book is the idea behind this folder; the chapter on the daily decision is where
`hub-decide` puts it to work. The two rooms next door are `forecasts/` (what your hub expects to
