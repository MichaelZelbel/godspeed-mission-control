---
name: connect-email
description: Connect the person's Gmail to the hub when they say "connect Gmail for me", "connect my email", "can you read my email", "set up email", "Gmail verbinden" or anything like it, and help an assistant or a second computer use a connection that already exists. Checks what already works first, starts the hub's own setup, and keeps the app password out of the chat. Also for "email stopped working" and "disconnect my Gmail".
---

## What this is

The person wants their assistants to search their Gmail, read a message they pick, and save
draft replies in Gmail's Drafts. The hub's mail tool (`hub-mail`) does that through Himalaya, a
...
- **Nothing is sent.** Saving a draft the person asked for needs no extra approval. They press
  Send in Gmail.
- **Gmail is not the hub's own address.** `account: "hub"` is a different mailbox. Never answer a
...
1. **Look first.** Call `mail_status` (the `hub-mail` tools). If `gmail` says connected, say so
...
2. **A connection they already have.** If this assistant already has its own Gmail connector
   ...
   it. The hub's connection is for when they want every assistant, or their server's jobs, to use
   one connection. Do not replace something that works.
3. **No hub tools here.** If you have no `hub-mail` tools and cannot run commands (a chat in a
...
6. **Start it.** Call `mail_connect`, or run `hub-mail connect gmail-imap` if you run commands.
...
- **Google says app passwords are not available for the account:** that account cannot use this
  route. Nothing is broken. Offer pasting, or forwarding to the hub's own address (Chapter 29).
...
- **An assistant on the same computer has no mail tools:** run `hub-mail setup --check`, which
  starts the tool the way each assistant does, then restart that assistant.
- **Disconnect:** `hub-mail disconnect gmail`, then they remove the app password on Google's
  page. Only removing it at Google stops every copy.

## A second computer, or a desktop app using the server's Gmail

Copying the hub folder does not copy the Gmail connection; the password stays on the computer
...
- **They have a server with Gmail connected on it** (step 4): pair this desktop with it, so the
  ...
  1. On the laptop run `hub-mail pair request`. It prints one line starting `hub-mail pair approve`.
  2. The person pastes that whole line into the server's terminal (the one in the provider's web
     page is fine), logged in as the account that runs the hub (a book server: first
     `sudo -iu ai`). It prints one line starting `hub-mail pair finish`.
...
it, then pair the laptop, then `hub-mail disconnect gmail` on the laptop. Never the other order.
