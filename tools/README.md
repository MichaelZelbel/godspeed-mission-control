...
|---|---|
| `hub-compile-rules` | Rebuild the compiled rule block from rules/. `--check` inspects whether it is current. See Chapter 17. |
| `hub-check-brief FILE` | Check delivery-format problems, including unsupported local-file instructions. It does not check factual truth or word count. See Chapter 22. |
| `hub-due` | Inspect the full deadline list. The daily cycle runs `hub-due check` before `hub-due today`. See Chapter 27. |
| `hub-due done NAME` | Manually close a completed task. |
| `hub-due drop NAME --yes` | Deliberately remove a task and its history. |
| `hub-check-keys` | Inspect stored versus locally available credentials and recorded expiry. It is not a successful login to every service. See Chapter 30. |
| `hub-notebook-sync --verbose` | Inspect an explicit sync run, selected uploads, downloaded records and failures. See Chapter 29. |
| `hub-prompt-archive` | Collect supported authorized local conversation text if enabled. Collection defaults off; filtering is not a complete privacy guarantee. See Chapters 19, 30 and 34. |
