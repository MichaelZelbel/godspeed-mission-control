...
cmds.file = (a) => {
  ...
  if (!oneLine(a.source)) die('--source is required: where this goal was said (a date and a channel), so it is never a goal Mission Control invented');
...
const PLAYBOOK_SECTIONS = [
  ...
  ['## Mission Control steps', '(what a mission control can do without the person: research, build the measurement, compute, find, draft, prepare)'],
  ['## Person steps', '(what only the person can do, and what Mission Control prepares for each)'],
...
// the first work, and it is work Mission Control can do alone.
