'use strict';
//
// hub-cards.js - the one card format that goals.js, forecast.js and work.js all share.
//
...
//
// WHERE IS MY HUB? `HUB_ROOT` if it is set, else the first folder at or above the folder you
// are standing in that looks like a hub (it has a `rules/` or an `observations/` folder), else
// the hub this machine was joined to, recorded in `~/.hub/device.env`. So typing one of these
// inside your hub always works, and a job the schedule starts with almost no environment can
...
function recordedHub() {
  try {
    ...
    const t = fs.readFileSync(path.join(home, '.hub', 'device.env'), 'utf8');
    const m = t.replace(/\r/g, '').match(/^[ \t]*(?:export[ \t]+)?HUB_DIR=(.*)$/m);
    const d = m ? m[1].trim().replace(/^["']|["']$/g, '') : '';
    return isHub(d) ? d : null;
  } catch (e) { return null; }
}
function hubRoot(fromFile) {
  if (process.env.HUB_ROOT && isHub(process.env.HUB_ROOT)) return process.env.HUB_ROOT;
  // These programs are installed OUTSIDE your hub, so walking up from this file only finds one
  // on a machine where they happen to sit inside a hub. The folder you are standing in is the
...
// The rest are yours. Put them in `rules/machine-words.txt` in your hub, one per line. A plain
...
function machineWords(hub) {
  const out = BASE_MACHINE.slice();
  const text = readText(path.join(hub, 'rules', 'machine-words.txt'));
...
function sibling(fromFile, hub, name) {
  const home = process.env.HOME || process.env.USERPROFILE || '';
  const dirs = [
    path.dirname(fs.realpathSync(fromFile)),
    path.join(home, '.local', 'bin'),
    path.join(hub, 'agents', 'hub-cli'),
  ];
  for (const d of dirs) {
...
function runNode(fromFile, hub, name, args, env) {
  const bin = sibling(fromFile, hub, name);
  ...
  const r = spawnSync(process.execPath, [bin].concat(args), {
    encoding: 'utf8', env: Object.assign({}, process.env, { HUB_ROOT: hub }, env || {}), timeout: 60000,
  });
  return { ok: r.status === 0, missing: false, out: r.stdout || '', err: r.stderr || '' };
}

module.exports = {
  hubRoot, today, isoDate, addDays, daysBetween, readText, oneLine, die, say, parseArgs, flag, alias,
