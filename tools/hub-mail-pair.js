/*
 * hub-mail-pair.js - let a desktop computer use the mail connection that lives on your server.
 *
 ...
 *   1. On the desktop: `hub-mail pair request` makes a key for this device only (its private
 ...
 *      is fine), as the account that runs the hub: `hub-mail pair approve <request>`. It adds the
 ...
 *   3. On the desktop: `hub-mail pair finish <receipt>`. The receipt must answer this device's own
 *      request. Its host key is written into a known_hosts file of hub-mail's own, host checking
 *      stays strict, and the mail tool is started over SSH and asked for its status before
 *      anything is switched. Then this computer's `hub-mail mcp` carries every call to the server.
 *
...
const sshDir = () => path.join(home(), ".hub", "mail", "ssh");
const routeFile = () => path.join(home(), ".hub", "mail", "route.json");
const REQ = "hubmail-pair-1.", REC = "hubmail-receipt-1.";
...
function request({ device } = {}) {
  ...
  if (!fs.existsSync(key)) {
    const r = spawnSync(keygen, ["-q", "-t", "ed25519", "-N", "", "-C", "hub-mail:" + name, "-f", key], { encoding: "utf8", windowsHide: true });
...
function sshArgs(r) {
  return ["-T", "-i", path.join(sshDir(), "id_ed25519"), "-o", "IdentitiesOnly=yes", "-o", "BatchMode=yes",
    ...
    "-o", "ConnectTimeout=15", "-o", "ServerAliveInterval=30", "-p", String(r.port), `${r.user}@${r.host}`, "hub-mail-mcp"];
}

// One MCP conversation over the route: initialize, then call mail_status. "" when it answered.
function probe(r) {
  const lines = [{ jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "hub-mail-pair", version: "1" } } },
...
function finish(line) {
  ...
  if (!pending) throw new Error("this computer has no pairing request waiting. Start with: hub-mail pair request");
...
// The desktop's `hub-mail mcp` when a route exists: a straight pipe to the server's mail tool.
...
function relay(r) {
  ...
  child.stderr.on("data", d => process.stderr.write("hub-mail (server): " + d));
...
function forcedCommand() {
  const tool = fs.existsSync(path.join(os.homedir(), ".local", "bin", "hub-mail.js")) ? path.join(os.homedir(), ".local", "bin", "hub-mail.js") : path.join(__dirname, "hub-mail.js");
...
function approve(line) {
  ...
  if (process.getuid && process.getuid() === 0) throw new Error("this is the root account. Run it as the account that runs your hub (for a book server: sudo -iu ai hub-mail pair approve ...), so the key opens that account's mail tool and not root's.");
  ...
  const hk = [process.env.HUB_MAIL_TEST_HOST_KEY_FILE && process.env.HUB_MAIL_TEST_HOST_KEY_FILE.includes("hub-mail-pair-test") ? process.env.HUB_MAIL_TEST_HOST_KEY_FILE : "/etc/ssh/ssh_host_ed25519_key.pub"].map(f => { try { return fs.readFileSync(f, "utf8").trim().split(/\s+/).slice(0, 2).join(" "); } catch (e) { return ""; } }).filter(k => KEY.test(k));
...
function list() {
  ...
  return text.split(/\r?\n/).map(l => (l.match(/ hub-mail:([A-Za-z0-9._-]+)$/) || [])[1]).filter(Boolean);
}
function remove(device) {
  if (!DEVICE.test(device || "")) throw new Error("which device? hub-mail pair list names them");
  const f = authorizedKeys();
  let text = ""; try { text = fs.readFileSync(f, "utf8"); } catch (e) { return false; }
  const lines = text.split(/\r?\n/).filter(Boolean);
  const kept = lines.filter(l => !l.endsWith(" hub-mail:" + device));
  if (kept.length === lines.length) return false;
  fs.writeFileSync(f + ".hub-mail.tmp", kept.join("\n") + "\n", { mode: 0o600 });
  fs.renameSync(f + ".hub-mail.tmp", f);
  return true;
}
