 ...
 * (`desktop: true`, which `hub-mail connect gmail-imap` passes) and Desktop is installed here,
 ...
 *
 * It leaves alone what is there: an existing hub-mail entry that works (yours, or an older one)
...
const installed = () => path.join(os.homedir(), ".local", "bin", "hub-mail.js");
...
function handshake(command, args, cwd, env) {
  const input = [
    { jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "hub-mail-setup", version: "1" } } },
...
function brokenPath(entry, cwd) {
  ...
  if (args.includes("-e")) return /\.local.*hub-mail\.js/.test(args.join(" ")) && !fs.existsSync(installed()) ? "the mail tool is not installed in ~/.local/bin" : "";
  const js = args.find(a => /hub-mail(\.js)?$/.test(a));
...
function claudeCode(hub, check) {
  ...
  if (doc.mcpServers[NAME]) return report("Claude Code", doc.mcpServers[NAME], hub, check);
...
function codex(check) {
  ...
  const before = readIf(file) || "";
  const m = before.match(/^\s*\[mcp_servers\.(?:"hub-mail"|hub-mail)\]\s*\r?\n([\s\S]*?)(?=^\s*\[|(?![\s\S]))/m);
  if (m) {
...
function hermes(check) {
  ...
  const before = readIf(file) || "";
  const m = before.match(/^\s{2}hub-mail:\s*\r?\n((?:\s{4}.*\r?\n?)*)/m);
  if (m) {
...
function wire({ check = false, hub = "", desktop = false } = {}) {
  const lines = [];
  let failed = false;
  const tool = fs.existsSync(installed()) ? installed() : path.join(__dirname, "hub-mail.js");
  const parts = [claudeCode(hub, check), codex(check), hermes(check)];
  ...
  if (!check) lines.push("Email itself stays off until you connect it: ask your assistant \"Connect Gmail for me\" (Chapter 30) for your mailbox, hub-mail connect agentmail (Chapter 29) for the hub's own address.");
  return { lines, failed };
}
