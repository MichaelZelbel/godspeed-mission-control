/*
 * mc-mail-gmail.js - your own Gmail for mc-mail: connect it once, read it, save drafts in it,
 * and send only a message you approved yourself.
 *
 * It is a module, not a command. mc-mail.js is the command and the MCP server; this file is
 * what they call for the "gmail" account. It sits beside them because the installer copies the
 * whole tools folder to one place.
 *
 * ONE CONNECTION FOR EVERY ASSISTANT. `mc-mail connect gmail` asks Google once, in your
 * browser, and keeps what Google hands back in your mission control's locked store
 * (secrets/mc-secrets.env.age), the same place the notebook key lives. Every assistant starts
 ...
 *     is you, pressing Send in Gmail on a draft the mission control saved. As an extra for people who like
 *     a terminal, a message can also go out after you approved the exact message yourself, by
 *     typing `mc-mail approve <code>`, within 30 minutes, once. No tool accepts "approved: true".
 ...
 * drafts the mission control wrote) lives in ~/.godspeed/mail/ on each computer, readable by you only.
 */
"use strict";
...
const AUTH_URL = process.env.GODSPEED_MAIL_GOOGLE_AUTH_URL || "https://accounts.google.com/o/oauth2/v2/auth";
const TOKEN_URL = process.env.GODSPEED_MAIL_GOOGLE_TOKEN_URL || "https://oauth2.googleapis.com/token";
const REVOKE_URL = process.env.GODSPEED_MAIL_GOOGLE_REVOKE_URL || "https://oauth2.googleapis.com/revoke";
const API = (process.env.GODSPEED_MAIL_GMAIL_API || "https://gmail.googleapis.com") + "/gmail/v1/users/me";
...
function writeStore(changes) {
  ...
  if (!fs.existsSync(key)) {
    ...
    if (made.status !== 0 || !fs.existsSync(key)) throw new Error("could not make a key for your mission control's locked store on this computer");
...
function shareStore(why) {
  ...
  if (!(git("remote").stdout || "").trim()) return "saved in your mission control's history (it has no remote to send it to)";
  const p = git("push", "-q");
...
async function accessToken(c) {
  ...
  if (r.status !== 200 || !r.body.access_token) {
    const e = new Error(r.body.error === "invalid_grant"
      ? "Google no longer accepts this connection (it was withdrawn, expired or the password changed). Reconnect: mc-mail connect gmail"
...
// ONE ATTACHMENT, FETCHED TO A PLACE OUTSIDE THE GODSPEED FOLDER.
// The mission control folder is a git repository that travels to the person's other computers and often to
// GitHub. A client's PDF must never end up in that history, so the copy goes to
// ~/.godspeed/mail/attachments/ on this computer, readable by the person only, and a place inside the
// godspeed folder is refused outright. The assistant gets the file's location, and for a text file
...
async function attachment(args, wrap) {
  ...
  const head = [`account: gmail (${c.GMAIL_ADDRESS})`, `message_id: ${m.id}`, `attachment: ${a.number} of ${att.length}`,
    ...
    `This is a copy outside your mission control folder, so it never enters your mission control's history. Do not copy it into the mission control folder unless the person asks. The original stays in Gmail; this copy is removed after ${KEEP_ATTACHMENT_DAYS} days.`,
...
function readAttachments(list) {
  ...
  for (const p of [].concat(list || [])) {
    ...
    if (godspeed && !full.startsWith(path.resolve(godspeed) + path.sep)) throw new Error(`attachment "${p}" is outside your mission control folder`);
...
function buildRaw(m) {
  const id = m.messageIdHeader || `<godspeedmail-${crypto.randomBytes(12).toString("hex")}@${(m.from.split("@")[1] || "godspeed.local").replace(/[>\s]/g, "")}>`;
  ...
  if (!m.attachments.length) raw = head.join("\r\n") + "\r\n" + bodyPart;
    ...
    const b = "godspeedmail-" + crypto.randomBytes(8).toString("hex");
...
// it in Gmail. The one rule is about YOUR edits: if a draft the mission control saved has been changed
// since (by you, or by anybody), the mission control never writes over it. It saves its new version as a
// separate draft and says so.
async function draft(args) {
  const c = credentials(); need(c);
  if (!canDraft(c)) throw new Error("gmail: this connection reads only, so it cannot save drafts. Give the person the reply as text. To let the mission control save drafts, they ask their assistant \"Connect Gmail for me\", which connects Gmail the current way.");
  let ctx = {};
  ...
  if (args.draft_id) {
    ...
    // In place only when nothing can be lost: the draft is exactly what the mission control last wrote, or
    ...
    if (curId && ((mine && curId === mine.message_id) || (args.base_version && curId === args.base_version))) {
      ...
      note = mine && curId === mine.message_id ? "updated the draft the mission control saved earlier" : "updated the draft, on top of the version you had edited";
    } else {
  ...
  return { account: "gmail", draft_id: saved.id, saved_in: "Gmail Drafts of " + c.GMAIL_ADDRESS, note,
    ...
    next: "Nothing was sent, and the mission control does not send. Tell the person the draft is waiting in Gmail Drafts: they read it there, change what they like, and press Send themselves." };
}
...
async function proposeSend(args) {
  const c = credentials(); need(c);
  if (!canDraft(c)) throw new Error("gmail: this connection reads only, so it cannot send. Send it yourself from Gmail, or reconnect with: mc-mail connect gmail");
  ...
  return { account: "gmail", proposal: code, status: "waiting for your approval", sent: false, expires: p.expires,
    ...
    ask_the_user: `Nothing is sent yet. To send exactly this message, type in a terminal on this computer: mc-mail approve ${code}  (it shows the whole message first; valid for ${APPROVAL_MINUTES} minutes). To drop it: mc-mail reject ${code}` };
}
...
// The only way anything is sent: a person runs `mc-mail approve <code>` in a terminal, sees
...
async function approve(code, confirm, opts = {}) {
  ...
  try {
    ...
    return { sent: false, why: "Gmail did not answer, so it is not known whether the message went. Run mc-mail approve " + code + " again: it checks your Sent mail first and never sends twice." };
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer(async (req, res) => {
      ...
      const finish = (msg, ok) => { res.writeHead(ok ? 200 : 400, { "Content-Type": "text/html; charset=utf-8" });
        res.end(`<!doctype html><meta charset="utf-8"><title>Godspeed mail</title><p style="font:16px sans-serif;margin:3em">${msg}</p>`); };
      ...
      finish("Done. Your mission control has its answer from Google. You can close this tab and go back to " + back + ".", true);
      resolve(r.body);
    });
    let redirect = "";
...
async function connect({ readOnly = false, ask, say, open, clientId, clientSecret, loginHint, back, fresh = false }) {
  ...
  if (!fresh) {
    clientId = clientId || before.GMAIL_CLIENT_ID || process.env.GODSPEED_MAIL_GOOGLE_CLIENT_ID;
    clientSecret = clientSecret || before.GMAIL_CLIENT_SECRET || process.env.GODSPEED_MAIL_GOOGLE_CLIENT_SECRET;
  }
  if (!clientId) {
    say("Your mission control needs the two lines Google gives you for a desktop app (the walkthrough: mail/gmail-setup.md in the kit).");
  ...
  voidProposals();
  cached = null;
  const shared = shareStore("mc-mail: connect Gmail (" + prof.emailAddress + ")");
...
// disconnect: the mission control stops at once; Google is asked to forget the permission too.
async function disconnect() {
  ...
  const shared = shareStore("mc-mail: disconnect Gmail");
  return { local: "disconnected: no assistant of this mission control can read or draft in that mailbox any more", google, shared,
...
async function status() {
  ...
  try {
    ...
    return { account: "gmail", address: prof.emailAddress, state: canDraft(c) ? "connected" : "reading only",
      ...
      note: canDraft(c) ? "Reading and drafting are on. The mission control sends nothing: you press Send in Gmail." : "Reading only. Drafts stay as text in the mission control.",
