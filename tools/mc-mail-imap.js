/*
 * mc-mail-imap.js - your Gmail for mc-mail, through Himalaya (a free mail program) and a
 * Google app password. No Google Cloud app, no connector company, no second computer.
 *
 * WHY THIS FILE IS HERE (the reviewed email plan, godspeed work/plans/email-strategy-2026-09-21.md).
 ...
 * already lets an ordinary mail program in with an app password, so the mission control uses one: Himalaya
 * 2.1.0, pinned by archive and SHA-256 (mc-mail-himalaya.json), talking IMAP to imap.gmail.com.
 * The person does the two things only they can do: make the app password on Google's page, and
 * type it into a window of this computer. The mission control does the rest.
 *
 * WHERE THINGS LIVE, all on THIS computer and never in the mission control folder or its Git history:
 *   ~/.godspeed/mail/imap/bin/        the pinned Himalaya program
 *   ~/.godspeed/mail/imap/state.json  which address, which Drafts folder, what was last verified (no secret)
 *   ~/.godspeed/mail/imap/account.toml  Himalaya's settings, rewritten from state.json on each use (no secret)
 *   ~/.godspeed/mail/imap/password.age  the app password, locked with age to a key of this folder
 *   ~/.godspeed/mail/imap/journal.jsonl every draft the mission control tried to save, and what became of it
 ...
 * godspeed looks for that Message-ID in Drafts; found means saved, not found means "uncertain", and
...
const WAIT_MS = Number(process.env.GODSPEED_MAIL_IMAP_WAIT_MS) || 45000;
const RUN_MS = () => Number(process.env.GODSPEED_MAIL_IMAP_TIMEOUT_MS) || 60000;
...
const supported = () => !!(asset() && (asset().tested || process.env.GODSPEED_MAIL_IMAP_UNTESTED === "1"));
...
async function install(say = () => {}) {
  const a = asset();
  if (!supported()) {
    throw Object.assign(new Error(`this kind of computer (${platformKey()}) has not been tested with the mission control's mail program yet, so the mission control does not install it here. You can still paste an email into the chat, or forward it to the mission control's own address.`), { kind: "unsupported" });
  }
  ...
  ensureBase();
  let tgz;
  if (process.env.GODSPEED_MAIL_HIMALAYA_ARCHIVE) tgz = fs.readFileSync(process.env.GODSPEED_MAIL_HIMALAYA_ARCHIVE);
  else {
...
function mailKey() {
  ...
  if (!keygen) throw new Error("the small program called age is not on this computer. The mission control installer fetches it: run it once, then try again.");
  ensureBase();
...
function writeConfig(dir, st, passwordFile) {
  const secret = [process.execPath, path.join(__dirname, "mc-mail-imap.js"), "secret", passwordFile];
  let server = GMAIL_IMAP, extra = "";
  // Only the tests point this elsewhere, and only at this computer.
  const t = process.env.GODSPEED_MAIL_IMAP_TEST_SERVER;
  if (t) {
    if (!/^imap:\/\/127\.0\.0\.1:\d+$/.test(t)) throw new Error("GODSPEED_MAIL_IMAP_TEST_SERVER may only name a stand-in on 127.0.0.1");
...
function runRaw(dir, args, { input = "", timeoutMs = RUN_MS() } = {}) {
  return new Promise(resolve => {
    const child = spawn(exePath(), ["-c", "./account.toml", "-a", ACCOUNT, "--json", "--log-level", "off", ...args],
      { cwd: dir, env: { ...process.env, GODSPEED_MAIL_IMAP_PIPE: "1" }, windowsHide: true });
...
function trouble(r, doing) {
  ...
  if (r.timedOut) { kind = "timeout"; msg = `Gmail did not answer within ${Math.round(RUN_MS() / 1000)} seconds while ${doing}`; }
  else if (r.cut) { kind = "too-big"; msg = "the answer from Gmail was larger than the mission control accepts"; }
  else if (/AUTHENTICAT|LOGIN failed|Invalid credentials|Application-specific password|Web login required|\[ALERT\]/i.test(text)) {
    kind = "auth";
    msg = "Gmail did not accept the app password. It may have been removed at Google, or the Google account password changed, which removes every app password. Reconnect: ask your assistant to connect Gmail again, or type mc-mail connect gmail-imap";
    ...
    kind = "mailbox"; msg = "Gmail does not have the folder the mission control expected (" + text.split(/\r?\n/)[0].slice(0, 160) + ")";
...
function notConnected() {
  return Object.assign(new Error("gmail: not connected on this computer. When the person wants it, they ask their assistant \"Connect Gmail for me\" (or type mc-mail connect gmail-imap). Until then they can paste an email, or forward it to the mission control's own address."), { kind: "not_configured" });
}
...
// ============================================================ message references
// Opaque to the assistant, precise to the mission control: which connection, which folder, which folder
...
function status() {
  ...
  if (!st || !st.address) {
    return { account: "gmail", route: "Himalaya on " + os.hostname(), state: "not connected", setup: supported() ? "available" : "not tested on this kind of computer",
      note: "Optional. When you want it, ask your assistant: Connect Gmail for me. Until then, paste an email or forward it to the mission control's address." };
  }
...
async function fetchRaw(st, ref) {
  ...
  if (m.size > MAX_READ_BYTES) throw new Error(`gmail: that message is ${Math.round(m.size / 1048576)} MB with its attachments, more than the mission control reads (${MAX_READ_BYTES / 1048576} MB). Open it in Gmail.`);
...
async function draft(args) {
  ...
  const domain = (st.address.split("@")[1] || "godspeed.invalid").replace(/[^A-Za-z0-9.-]/g, "");
  ...
  return withLock(["writer"], async () => {
    ...
    for (const o of earlier) {
      ...
      if (!args.after_checking_drafts) {
        return { account: "gmail", saved: "uncertain", sent: false,
          note: "An earlier try to save this same draft may or may not have reached Gmail, and the mission control could not confirm it either way. It did not try again, so Drafts does not get two copies.",
    ...
    m.messageId = `<godspeedmail.${op}@${domain}>`;
    ...
    const result = (saved, extra) => Object.assign({ account: "gmail", saved, sent: false, saved_in: "Gmail Drafts of " + st.address,
      ...
      next: "Nothing was sent, and the mission control does not send. Tell the person the draft is waiting in Gmail Drafts: they read it there, change what they like, and press Send themselves." }, extra || {});
    try {
      ...
      return { account: "gmail", saved: "uncertain", sent: false,
        note: "Gmail did not confirm the save (" + e.message + "), and the draft was not found in Drafts " + (found === false ? "just now" : "because Drafts could not be checked") + ". Gmail can take a moment to show a new draft. The mission control did not try again, so Drafts does not get two copies.",
...
function terminalCommand() {
  ...
  const tool = path.join(__dirname, "mc-mail.js");
...
async function connectHere({ ask, say, open = G.openBrowser }) {
  ...
  say("");
  say("Your mission control will be able to search your Gmail, read a message you pick, and save draft replies");
  ...
  say("  " + page);
  say("Sign in if Google asks. Type a name such as godspeed mail, click Create, and Google shows 16 letters.");
  ...
  say("turn them off. Type stop then; nothing on your account is changed by the mission control.");
  say("");
  ...
  try {
    for (let tries = 1; tries <= 3; tries++) {
      ...
      say(drafts ? `Drafts will be saved in your folder "${drafts}".` : "No Drafts folder was found, so the mission control can read but not save drafts.");
...
function secretMain(argv) {
  const src = argv[1];
  if (process.env.GODSPEED_MAIL_IMAP_PIPE !== "1" || process.stdout.isTTY) { console.error("mc-mail: this hands the mail password to the mail program only."); process.exit(2); }
  const inside = p => { const rel = path.relative(base(), path.resolve(p)); return rel && !rel.startsWith("..") && !path.isAbsolute(rel); };
  if (!src || !inside(src) || !/password\.age$/.test(src)) { console.error("mc-mail: unknown password file"); process.exit(2); }
  const pw = unlockPassword(src);
  if (pw === null) { console.error("mc-mail: the mail password could not be unlocked on this computer"); process.exit(1); }
...
if (require.main === module) {
  ...
  else { console.error("mc-mail-imap.js is part of mc-mail: use mc-mail connect gmail-imap"); process.exit(2); }
}
