...
/* WHERE IS MISSION CONTROL? This program is INSTALLED on the computer, like your assistant is, rather
...
/* WHAT MAKES A FOLDER A MISSION CONTROL, and why this is not one folder name.
 *
 * Until 2026-08-29 this asked one question: does it contain `memory/`. On 2026-08-21 Mission Control
 ...
 * folder name is a dependency on a layout Mission Control is allowed to change, so the question is now
 * asked of several names, plus the file where a mission control declares its own folders. The second is
 * worse: the starter mission control this kit INSTALLS ships `observations/` and no `memory/`, so the
 * harvester could not find Mission Control the kit itself had just created. It was broken for every new
 ...
 * on the strength of a missing folder. If someone has said where their mission control is, that is the
 ...
 *
 * WHY. Until 2026-08-29 the only thing a harvest put into Mission Control was archived prompts, and
...
/* The last mission control that worked, remembered on this machine. If mc-finding breaks tomorrow there
 * is no repository to write a receipt into, which is exactly when one is worth having, so a
 * failing run files its receipt in Mission Control it used last. Used for the receipt only, never to
...
function receipt(fields) {
  ...
  if (!godspeed) {
    /* The remembered mission control has to STILL BE THERE. A path that has been moved or deleted is a
...
function main() {
  ...
  if (ARGS.includes('--where')) {
    ...
    warn('I could not find your mission control folder. Set GODSPEED_DIR in ~/.godspeed/device.env to the folder '
       + 'your godspeed is in, or run the installer again.');
    return 1;
  }
  ...
  if (!GODSPEED) {
    const msg = 'I could not find your mission control folder, so there is nowhere to file what you have '
      ...
      + 'installer again.';
    warn(msg);
    receipt({ error: msg });        // into Mission Control this machine used last, if there was one
    return 1;
  }
  ...
  /* The run worked. Say so IN MISSION CONTROL, before the commit, so the receipt is part of the same
