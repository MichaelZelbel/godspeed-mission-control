...
#
# Runs entirely inside a throwaway mission control root. It never reads or writes the real goals/.
...
OUT="$(hg file --kind outcome --title "No source" 2>&1)"; check "a goal without a source is refused (never a goal Mission Control invented)" "$?" "1"
...
hg file --kind outcome --title "Mission Control as a real working system" --id godspeed --area godspeed --status adopted --importance high --source "priorities 2026-06-15" --deadline 2026-09-15 >/dev/null
...
OUT="$(hg question friends --text "Is five friends by the end of 2027 a goal you want Mission Control to work on?" 2>&1)"
...
for H in "## Who we model" "## What they do" "## In what order" "## What they track" "## Where it fails" "## Mission Control steps" "## Person steps" "## Unknown" "## Sources"; do
  check "  section $H present" "$(grep -c "^$H" "$TMP/$P")" "1"
done
