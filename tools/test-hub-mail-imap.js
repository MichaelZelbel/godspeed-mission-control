#!/usr/bin/env node
/*
 * The gate for hub-mail's Gmail route through Himalaya: the REAL pinned Himalaya 2.1.0 program,
 * talking to a stand-in for Gmail's IMAP door on 127.0.0.1 (test-hub-mail-imap-fixture.js).
 * Nothing here can reach a real mailbox: the address is invented, the password is invented, and
 * the settings refuse any server but 127.0.0.1. This is a simulation: it proves the hub's own
 ...
 *     hub's own inbox.
 *
 * The program comes from HUB_MAIL_HIMALAYA_ARCHIVE when set (a .tgz), else it is downloaded
 * once into the system's temp folder and checked against the same pinned hash.
 *
 * Usage: node tools/test-hub-mail-imap.js
 */
"use strict";
const fs = require("fs");
const os = require("os");
...
async function archive() {
  if (process.env.HUB_MAIL_HIMALAYA_ARCHIVE) return process.env.HUB_MAIL_HIMALAYA_ARCHIVE;
  if (!A) return "";
  const cache = path.join(os.tmpdir(), "hub-mail-test-" + A.asset);
...
async function main() {
  ...
  const W = fs.mkdtempSync(path.join(os.tmpdir(), "hub mail imap "));   // a space, on purpose
  ...
  for (const k of Object.keys(process.env)) if (/^GMAIL_|^AGENTMAIL_|^HUB_MAIL_/.test(k)) delete process.env[k];
  Object.assign(process.env, { HUB_MAIL_HOME: HOME, HUB_DIR: HUB, HUB_AGE_KEY: path.join(HOME, ".hub", "no-store-key"), HUB_MAIL_IMAP_TIMEOUT_MS: "20000" });

  const F = require("./test-hub-mail-imap-fixture.js");
  const f = await F.start();
  process.env.HUB_MAIL_IMAP_TEST_SERVER = "imap://127.0.0.1:" + f.port;
  ...
  process.env.HUB_MAIL_HIMALAYA_ARCHIVE = bad;
  ...
  process.env.HUB_MAIL_HIMALAYA_ARCHIVE = tgz;
  const exe = await I.install();
  const v = spawnSync(exe, ["--version"], { encoding: "utf8" });
  ok("2 the pinned archive installs Himalaya " + MANIFEST.version + " in the hub's own mail folder", v.stdout.includes("v" + MANIFEST.version) && exe.startsWith(base));
  ...
  // ---- the password helper
  const helper = spawnSync(process.execPath, [path.join(__dirname, "hub-mail-imap.js"), "secret", path.join(base, "password.age")], { encoding: "utf8", env: Object.assign({}, process.env, { HUB_MAIL_IMAP_PIPE: "" }) });
  ...
  ok("27 the saved message is whole: sender, recipient, encoded subject, own Message-ID, text", ap && /From: reader@example\.invalid/.test(ap.raw) && /To: "Nadia Park" <nadia@example\.invalid>/.test(ap.raw)
    && /Subject: =\?UTF-8\?B\?/.test(ap.raw) && /Message-ID: <hubmail\.[0-9a-f]+@example\.invalid>/.test(ap.raw) && Buffer.from(ap.raw.split("\r\n\r\n")[1].replace(/\s/g, ""), "base64").toString("utf8").includes("Viele Grüße"), ap && ap.raw);
  ...
  process.env.HUB_MAIL_IMAP_TIMEOUT_MS = "4000";
  ...
  process.env.HUB_MAIL_IMAP_TIMEOUT_MS = "20000";
  ...
  const mcp = lines => new Promise(res => {
    const c = spawn(process.execPath, [path.join(__dirname, "hub-mail.js"), "mcp"], { env: process.env });
  ...
  ok("43 with Gmail not connected, a search for Gmail says so and is never answered from the hub's own address", /gmail: not connected/.test(o2) && /account \\"hub\\" only when/.test(o2), o2);
  const retired = spawnSync(process.execPath, [path.join(__dirname, "hub-mail.js"), "connect", "gmail", "--guided"], { encoding: "utf8", env: process.env });
  ...
  const gs = spawnSync(process.execPath, [path.join(__dirname, "hub-mail.js"), "gmail-state"], { encoding: "utf8", env: process.env });
