#!/usr/bin/env node
/*
 * The gate for pairing a desktop with the server's mail tool (hub-mail-pair.js). It runs the two
 ...
 * Usage: node tools/test-hub-mail-pair.js
 */
"use strict";
const fs = require("fs");
const os = require("os");
...
const body = JSON.parse(Buffer.from(req.slice("hubmail-pair-1.".length), "base64url").toString());
...
ok("2 the private key stays on the desktop", fs.existsSync(path.join(DESK, ".hub", "mail", "ssh", "id_ed25519")));

if (process.platform !== "win32" && !(process.getuid && process.getuid() === 0)) {
  const realHome = process.env.HOME;
  process.env.HOME = SERVER;
  const hk = path.join(W, "hub-mail-pair-test-hostkey.pub");
  fs.writeFileSync(hk, KEY + " root@server\n");
  process.env.HUB_MAIL_TEST_HOST_KEY_FILE = hk;
  ...
  ok("4 the new key can only start the mail tool: restrict and a forced command", new RegExp('^restrict,command="\\S+ \\S+hub-mail\\.js mcp" ssh-ed25519 \\S+ hub-mail:laptop-one$', "m").test(text), text);
  const again = P.approve(req);
  ok("5 a device that pairs again replaces its key instead of adding a second", (fs.readFileSync(ak, "utf8").match(/hub-mail:laptop-one/g) || []).length === 1 && again.kept === 1);
  ok("6 a bad request is refused and nothing is written", /not valid/.test(err(() => P.approve("hubmail-pair-1." + b64({ v: 1, nonce: "x", device: "a", key: "ssh-rsa AAAA" })))) && /damaged|not a pairing request/.test(err(() => P.approve("rm -rf /"))));
  const rec = JSON.parse(Buffer.from(r.receipt.slice("hubmail-receipt-1.".length), "base64url").toString());
  ...
  ok("8 a receipt for another request is refused", /different request/.test(err(() => P.finish("hubmail-receipt-1." + b64(Object.assign({}, rec, { nonce: "0".repeat(32) }))))));
  ok("9 a receipt with no usable host key is refused", /no usable host key/.test(err(() => P.finish("hubmail-receipt-1." + b64(Object.assign({}, rec, { host_keys: ["ssh-ed25519 short"] }))))));
  ok("10 a receipt naming a strange address is refused", /not valid/.test(err(() => P.finish("hubmail-receipt-1." + b64(Object.assign({}, rec, { host: "a;rm -rf ~" }))))));
  const noAnswer = err(() => P.finish("hubmail-receipt-1." + b64(Object.assign({}, rec, { host: "127.0.0.1", port: 1 }))));
