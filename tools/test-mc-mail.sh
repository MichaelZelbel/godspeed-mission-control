#!/usr/bin/env bash
# The gate for mc-mail: the one mail tool every assistant of the mission control uses.
#
# WHY THIS FILE IS HERE. mc-mail hands text written by strangers to an assistant. Its promises
...
const hostile = { message_id: '<m1@x>', timestamp: '2099-01-01T08:00:00Z', from: 'Nadia <nadia@example.com>',
  to: ['godspeed@agentmail.to'], subject: 'Cards 8 to 12', labels: ['received'], preview: 'Where are they?',
...
const s = http.createServer((req, res) => {
  ...
  if (u.pathname === '/v0/inboxes/godspeed%40agentmail.to' || u.pathname === '/v0/inboxes/godspeed@agentmail.to') return out(200, { inbox_id: 'godspeed@agentmail.to' });
...
mcp() {  # feed newline-delimited JSON-RPC, give the server a moment, collect its answers
  { printf '%s\n' "$@"; sleep 2; } | "$NODE" "$HERE/mc-mail.js" mcp 2>&1
}
...
contains "1 initialize answers with its name" "$OUT" '"name":"mc-mail"'
...
OUT="$(mcp '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"mail_read","arguments":{"account":"godspeed","message_id":"<m1@x>"}}}')"
...
OUT="$(mcp '{"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"mail_search","arguments":{"account":"godspeed","query":"cards"}}}')"
lacks    "6 search does not return the mission control's own sent mail" "$OUT" '<s1@x>'

OUT="$(AGENTMAIL_READ_KEY="" "$NODE" "$HERE/mc-mail.js" status 2>&1)"
contains "7 without a key: gmail and godspeed are simply not listed as connected" "$OUT" 'gmail: not connected'
lacks    "7b and nothing claims a connection" "$OUT" 'godspeed: connected'
OUT="$(AGENTMAIL_READ_KEY="wrong-key" "$NODE" "$HERE/mc-mail.js" status 2>&1)"; RC=$?
...
OUT="$(GODSPEED_MAIL_AGENTMAIL_API="http://127.0.0.1:1" "$NODE" "$HERE/mc-mail.js" status 2>&1)"
