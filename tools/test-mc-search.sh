#!/usr/bin/env bash
# The gate for mc-search: ask the notebook first, and never need it.
#
# WHY THIS FILE IS HERE. mc-search is what AGENTS.md tells an assistant to run before it
# says "that is not in your mission control". So it has two promises, and both are about the bad day:
#
...
#   2. What comes back is always a file that is really in the mission control, and never AGENTS.md.
...
export HOME="$W/home" USERPROFILE="$W/home" GODSPEED_DIR="" GODSPEED_AGE_KEY="$W/home/.godspeed/no-such-key"
# Every case below is a reader who said yes to the mission control copy, unless it says otherwise:
# without that yes, mc-search never asks the notebook at all.
export GODSPEED_NOTEBOOK_MIRROR=1
...
hs() { "$NODE" "$HERE/search.js" --godspeed "$H" "$@" 2>&1; }
...
const srv = http.createServer((req, res) => {
  ...
  if (q.includes("oldshape")) {
    return send(200, JSON.stringify({ notes: [
      { title: "AGENTS.md", folder_path: "godspeed", source_app: "godspeed", similarity: 0.9 },
      { title: "profile/dentist.md", folder_path: "godspeed/profile", source_app: "godspeed", similarity: 0.8 },
      { title: "2026-01-05 Changed dentist, because the old one retired.", folder_path: "godspeed", source_app: "godspeed" },
      { title: "A note I wrote on my phone", folder_path: "Health", source_app: "menerio" },
    ] }));
  }
  send(200, JSON.stringify({ mode: "hybrid", results: [
    { title: "AGENTS.md", folder_path: "godspeed", source_app: "godspeed", source_id: "AGENTS.md", similarity: 0.95, snippet: "The dentist, the budget" },
    { title: "profile/dentist.md", folder_path: "godspeed/profile", source_app: "godspeed", source_id: "profile/dentist.md", similarity: 0.91, snippet: "Dr. Aydin, Tuesday at nine." },
    { title: "profile/deleted-last-week.md", folder_path: "godspeed/profile", source_app: "godspeed", source_id: "profile/deleted-last-week.md", similarity: 0.9, snippet: "gone" },
    { title: "2026-01-05 Changed dentist, because the old one retired.", folder_path: "godspeed", source_app: "godspeed", source_id: "decisions.md#2026-01-05-changed-dentist", similarity: 0.7, snippet: "Changed dentist, because the old one retired." },
    { title: "escape", folder_path: "godspeed", source_app: "godspeed", source_id: "../outside.md", similarity: 0.6, snippet: "x" },
  ] }));
});
...
if [ "$rc" = "0" ] && [ "$(echo "$out" | sed -n 1p)" = "profile/dentist.md" ] \
  ...
  ok "a hit is one block: the mission control path, the title, the snippet"
else
...
lacks "a path that points outside the mission control is not believed" "$out" "outside"
...
lacks "  and a note you wrote yourself is not passed off as a mission control file" "$out" "on my phone"
...
start=$(date +%s)
out="$(MENERIO_API_KEY=test-key GODSPEED_SEARCH_TIMEOUT_MS=1200 hs slow dentist)"; rc=$?
took=$(( $(date +%s) - start ))
...
# A notebook is connected, but this reader never put a copy of the mission control into it. Then there is
# nothing to ask the notebook, and no trip over the network to hear "nothing".
out="$(MENERIO_API_KEY=test-key GODSPEED_NOTEBOOK_MIRROR=0 MENERIO_BASE_URL="http://127.0.0.1:1" hs dentist)"
contains "with the mission control copy switched off, the files answer and the last line says why" "$out" "source: local files (your mission control is not copied to Menerio"
lacks "  and it never tried the network" "$out" "not reached"
out="$(MENERIO_API_KEY=test-key GODSPEED_NOTEBOOK_MIRROR= MENERIO_BASE_URL="http://127.0.0.1:1" hs dentist)"
contains "and no answer at all counts as no" "$out" "source: local files (your mission control is not copied to Menerio"
...
if "$PY" -c "pass" >/dev/null 2>&1; then
  ...
  js="$("$NODE" "$HERE/search.js" --godspeed "$H" --list-files | tr -d '\r')"
...
print("\n".join(ns.godspeed_markdown_files(pathlib.Path(sys.argv[2]))))
PARITYEOF
)"
  if [ -n "$js" ] && [ "$js" = "$py" ]; then
...
PARITYEOF
    ...
    if [ -n "$js" ] && [ "$js" = "$py" ] && ! echo "$js" | grep -q "goals/\|fresh.md\|node_modules"; then
      ...
      bad "the two lists disagree in a git godspeed" "js: $js | py: $py"
    fi
  fi
else
