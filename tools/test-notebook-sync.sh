...
mkdir -p "$W/hub/observations" "$W/hub/skills/plan-my-day" "$W/hub/profile" "$W/hub/rules" \
         "$W/hub/goals" "$W/hub/dev/some-project"
printf 'You proofread before sending.\n' > "$W/hub/observations/quirk.md"
printf '# Plan my day\n'                 > "$W/hub/skills/plan-my-day/SKILL.md"
printf '# About me\n'                    > "$W/hub/profile/about-me.md"
printf '# Ask first\n'                   > "$W/hub/rules/when-in-doubt-ask.md"
printf '# Health\n'                      > "$W/hub/goals/health.md"
printf '# Manual\n'                      > "$W/hub/AGENTS.md"
printf '# My projects\n'                 > "$W/hub/dev/README.md"
printf '# A project\n'                   > "$W/hub/dev/some-project/README.md"
...
# A hub where .claude/skills is a real second copy (Windows without links, or a hub
# somebody copied by hand) must not send every skill twice. The visible name wins.
mkdir -p "$W/twohub/skills/plan-my-day" "$W/twohub/.claude/skills/plan-my-day"
printf '# Plan my day\n' > "$W/twohub/skills/plan-my-day/SKILL.md"
printf '# Plan my day\n' > "$W/twohub/.claude/skills/plan-my-day/SKILL.md"
twoplan="$(MENERIO_API_KEY="" "$PY" "$HERE/notebook-sync.py" --repo-root "$W/twohub" 2>&1)"
...
[ -z "$missing" ] && ok "the whole hub is mirrored: AGENTS.md, profile/, rules/ and goals/ are in the plan" \
...
# The real hub is a git folder, and what git tracks is what is mirrored: .gitignore is
# the reader's own list of what stays home. A file git ignores, and a scratch file nobody
# committed, must both stay out. dev/README.md IS tracked in the starter hub, and stays
# out anyway.
if command -v git >/dev/null 2>&1; then
  ...
  if echo "$gplan" | grep -q "would create profile/about-me.md" \
    ...
    ok "in a git hub, what git tracks is sent"
  else
  ...
  if echo "$gplan" | grep -q "private/plan.md\|scratch/untracked.md\|dev/README.md"; then
    ...
    ok "in a git hub, ignored files, uncommitted files and dev/ stay home"
  fi
else
...
[ "$(echo "$lines" | grep "^AGENTS.md\|^goals/\|^world/" | grep -c "This is a copy of the hub file at .*Change the file in the hub, never this note")" = "3" ] \
  && ok "everything else says it is a copy, and to change the file and never the note" || bad "the neutral line is missing" "$lines"

# ---- a hub that already synced sends nothing again --------------------------------------
# The ids and the first line of a note are what the cache is keyed and hashed on. The
# strings below are the ones the program shipped with BEFORE the whole hub was mirrored,
...
resend="$("$PY" - "$HERE/notebook-sync.py" "$W/hub" <<'OLDEOF'
import hashlib, importlib.util, pathlib, sys
spec = importlib.util.spec_from_file_location("ns", sys.argv[1])
ns = importlib.util.module_from_spec(spec); spec.loader.exec_module(ns)
OLD = {
    "observations/": "This is a file from your hub at {path}. It is text a machine "
                     "wrote, which makes it a guess and not something you said.",
    "skills/": "This is a file from your hub at {path}. It was mostly written by a "
               "machine and kept because you found it useful.",
    "decisions.md": "This is a file from your hub at {path}. You wrote or decided this.",
}
    ...
    for prefix, line in OLD.items():
        if d.source_path.startswith(prefix):
            ...
            state[d.doc_id] = {"note_id": "n-" + d.doc_id,
                               ...
                               "folder": "hub/" + parent if parent else "hub"}
...
echo "$resend" | grep -q "^OLDIDS decisions.md#2026-01-05-chose-one-ai-subscription-instead-of-two-because decisions.md#2026-02-11-named-the-folder-hub-so-every-tool-calls-it-the decisions.md#2026-03-01-moved-the-notebook-key observations/quirk.md skills/plan-my-day/SKILL.md$" \
...
echo "$resend" | grep -q "^UPDATE 0 TRASH 0$" \
  && ok "an already-synced hub updates and trashes nothing it sent before" || bad "old notes would be sent again" "$resend"
...
# document is created, in a folder under hub/. Second run: not one request that writes.
...
if [ -s "$W/stub.port" ]; then
  ...
  if [ "$(echo "$first" | head -1)" = "9" ] && echo "$first" | grep -q "^POST AGENTS.md -> hub$" \
    ...
    ok "first run over the wire: nine notes created, each in its folder under hub/"
  else
