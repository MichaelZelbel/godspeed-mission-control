...
#
# WHY THIS FILE IS HERE AND NOT IN A MISSION CONTROL. The two programs in tools/ are what the book means
# by "a program fills it". They used to live in one person's private mission control, and so did their
...
# The fixture mission control is built the way Mission Control this kit INSTALLS is built: observations/, and no
# memory/. It made a memory/ until 2026-08-29, which is why this suite stayed green through the
# eight days both tools were unable to find a real mission control at all (see the note in prompt-harvest.js).
...
#
# WHY THESE FIVE EXIST. On 2026-08-10 the drawer held what he had typed to one bot, Mission Control
...
# 27. The bot's reply is never a PROMPT, and the sessions where Mission Control gives the bot work
...
# 42-45. CAN THE PAIR STILL FIND A MISSION CONTROL? (2026-08-29)
#
# Everything above this line tests what the archive does once it knows where Mission Control is. For
# eight days the answer was that it never got that far: prompt-harvest.js decided a folder was
# a mission control only if it held `memory/`, Mission Control renamed that folder on 2026-08-21, and all three of
# Michael's machines went quiet the same day with no error anyone would see. The starter mission control
...
if command -v node >/dev/null 2>&1 && [ -f "$HARV" ]; then
  ...
  # `--where` prints Mission Control it resolved and touches nothing, so mc-finding can be tested
  ...
  findable "$W/godspeedcheck/new" \
    ...
    || bad "42 Mission Control this kit installs is not recognised as a mission control" "$(cat "$W/hc.err")"
  ...
  # A run that cannot find a mission control has nowhere to file a receipt - unless it remembers Mission Control
  ...
  rcpt | grep -q '"ok": true' \
    && ok "46 a successful run says so in Mission Control, in its own file" \
  ...
  # is no repository to write into then, so the machine remembers Mission Control it used last and
