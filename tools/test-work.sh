...
"$NODE" -e 'const f=require("fs"),p=process.argv[1];f.writeFileSync(p,f.readFileSync(p,"utf8").replace(/^KIND: .*\n/gm,""))' "$TMP/work/W-20260913-03.md"
OUT="$(hw check 2>&1)"; check "an old card with no KIND is read as do, not a problem" "$?" "0"
"$NODE" -e 'const f=require("fs"),p=process.argv[1];f.writeFileSync(p,f.readFileSync(p,"utf8").replace(/^KIND: do$/m,"KIND: ponder"))' "$TMP/work/W-20260913-02.md"
...
"$NODE" -e 'const f=require("fs"),p=process.argv[1];f.writeFileSync(p,f.readFileSync(p,"utf8").replace(/^KIND: ponder$/m,"KIND: do"))' "$TMP/work/W-20260913-02.md"
"$NODE" -e 'const f=require("fs"),p=process.argv[1];f.writeFileSync(p,f.readFileSync(p,"utf8").replace(/^OUTWARD: no/m,"OUTWARD: yes"))' "$TMP/work/W-20260913-01.md"
