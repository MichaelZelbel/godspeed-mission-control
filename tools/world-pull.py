#!/usr/bin/env python3
"""Pull Menerio's World down into the mission control's world/ folder as markdown.
...
def render_entity(record, existing_text=None):
    ...
    When the mission control already has a file for this thing, only one line is added to
    it: menerio_id. Everything the mission control wrote stays exactly as it was, because
...
def match_entity_file(record, existing, claimed):
    """Find the mission control's file for this Menerio entity, or None for a new one.

    Menerio calls him "Michael Zelbel" and the mission control file is michael.md, so a slug
